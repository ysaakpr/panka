# How Development Teams Use Deployer

A visual guide showing exactly how teams use the deployer CLI tool.
...
```bash
# Alice installs CLI
alice@laptop:~$ curl -sSL https://deployer.io/install.sh | sh
Downloading deployer v1.0.0...
✓ Installed to /usr/local/bin/deployer

alice@laptop:~$ deployer version
deployer version 1.0.0

# Alice configures backend
alice@laptop:~$ deployer init
...
✓ Configuration saved to /home/alice/.deployer/config.yaml

# Verify configuration
alice@laptop:~$ cat ~/.deployer/config.yaml
version: v1
backend:
  type: s3
  region: us-east-1
...
# Initialize stack
alice@laptop:~/deployment-repo/stacks/notification-platform$ deployer stack init
...
```bash
alice@laptop:~/deployment-repo$ deployer validate --stack notification-platform
...
```bash
alice@laptop:~/deployment-repo$ deployer plan \
  --stack notification-platform \
...
├── Connecting to S3: company-deployer-state ✓
...
# Now actually deploy
alice@laptop:~/deployment-repo$ deployer apply \
  --stack notification-platform \
  --environment development \
  --var VERSION=v1.0.0
...
Next steps:
  ...
  • Logs: deployer logs --component email-service/api --follow
  • Status: deployer status --stack notification-platform
────────────────────────────────────────────────
```
...
bob@laptop:~/deployment-repo$ deployer apply \
  --stack notification-platform \
  --environment development \
  --var VERSION=v1.0.1
...
# Deploy
carol@laptop:~/deployment-repo$ deployer apply \
  --stack notification-platform \
  --environment development \
...
4. **Deploy with one command** (5 minutes)
   ```bash
   deployer apply --stack my-stack --environment dev --var VERSION=v1.0.0
   ```

### What Deployer Does Automatically

When you run `deployer apply`:
...
**A**: No. Just the `deployer` binary. It talks directly to AWS.
...
**A**: No. You just write YAML. Deployer translates to Pulumi internally.
...
```bash
# Deploy to dev
deployer apply --stack notification-platform --environment development --var VERSION=v1.0.0

# Deploy to production
deployer apply --stack notification-platform --environment production --var VERSION=v1.0.0
...
1. **One-Time Setup** (10 minutes)
   - Install CLI: `curl -sSL deployer.io/install.sh | sh`
   - Configure: `deployer init` (provide S3 bucket & DynamoDB table from platform team)
   - Done!
...
3. **Deploy** (5 minutes)
   - Build Docker image
   - Push to ECR
   - Run: `deployer apply --stack your-stack --environment dev --var VERSION=v1.0.0`
   - Done!

4. **Daily Updates** (5 minutes)
   - Make code changes
   - Build new version
   - Run: `deployer apply --var VERSION=v1.0.1`
   - Monitor with `deployer status` and `deployer logs`
